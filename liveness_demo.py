# USAGE
# python liveness_demo.py --model liveness --le le.pickle
	...
	while True:
		...
		for i in range(0, detections.shape[2]):
			...
			if confidence > args["confidence"]:
				...
				endX = min(w, endX)
				endY = min(h, endY)
				checker = [endX - startX, endY - startY, 0]
				if min(checker) == 0:
					# Lay vung khuon mat
		...
		if key == ord("q"):
			break
	Re_turn = {'videoname' : video,'detect' : ketqua}
	print(Re_turn)
	# do a bit of cleanup
	vs.release()
...
Folder_input = 'D:/hatt6/video/public_test_2/videos/'
Bangall = pd.DataFrame(columns=['videoname','detect'])
for file in os.listdir(Folder_input):
    if file.endswith('.mp4'):
        path_vid = Folder_input + file
        Data_L = run_the_wild_thing(path_vid)
        Bangall = Bangall.append(Data_L, ignore_index = True)
Bangall.to_csv("ketquadudoan.csv")